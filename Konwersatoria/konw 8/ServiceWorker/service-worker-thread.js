...
self.addEventListener('install',
    (e) => {
        ...
        e.waitUntil(
            // CacheStorage - dostępne globalnie, nie tylko w SW
...
self.addEventListener('fetch',
    e => {
        console.warn('[SW] fetch event.request', e.request)
        ...
        e.respondWith(
            caches.match(e.request)
                .then(resp => {
                    ...
                    return fetchData(e.request)
                })
                .catch(() => caches.match('./img/offline.png'))
